...
shuflabels = truelabels(randperm
...
for fi=1:num_frex
    ...
    % Don't forget to downsample by selecting only the 
    tf(1,fi,:,:) = ; % (note: no averaging b/c keeping all single-trial data!)
...
% first Z-score the real map against the H0 distribution
zmap = 
...
%  We need to create a distribution of cluster sizes
...
for permi = 1:n_permutes
    ...
    threshimg(abs(threshimg)<zval) = 999; % (999?!? really?)
...
% find cluster threshold (need stats toolbox for this line, otherwise find an alternative solution below!)
...
for permi = 1:n_permutes
    ...
    max_vals(permi,:) = % store the largest negative and largest positive values
end
...
% threshold real data
zmap = diffmap;
% set all pixels between the two thresholds to 0
zmap = 0;
...
time1 = [100 500]; % do these seem like good choices?!!?
time2 = [233 555];
