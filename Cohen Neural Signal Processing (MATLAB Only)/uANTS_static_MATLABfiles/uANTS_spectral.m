...
disp([ 'Real part is ' num2str(real(z)) ' and imaginary part is ' num2str(imag) '.' ])
...
figure(3), clf
plot(real(z),(z),'s','markersize',12,'markerfacecolor','k')
...
title([ 'Rectangular: [' num2str(real(compnum)) ' ' num2str(imag(compnum)) 'i ], ' ...
        'Polar: ' num2str(mag) 'e^{i' num2str(phase) '}' ])
...
% plot in 3D
subplot(212)
plot3(time,real(csw),real(csw),'k','linew',3)
...
amp1 = sqrt( ^2 + ^2 );% hint: use the functions "imag" and "real"
amp2 =
amp3 =
...
alphapower = mean(chanpowr(:,freqidx)),2);
...
signalX2 = fft(signal   % zero-pad to 10 plus length of signal!
signalX3 = fft(signal;  % zero-pad to 100 plus length of signal!
